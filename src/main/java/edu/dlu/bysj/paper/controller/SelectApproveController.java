...
public class SelectApproveController {
    ...
    public CommonResult<List<UnselectStudentVo>> unChooseTopicStudent(@Valid UnselectStudentQuery query) {
        List<UnselectStudentVo> unselectStudentVos = topicService.unChooseStudentList(query.getStudentNumber(), query.getStudentName(), query.getYear(), query.getMajorId());
    ...
    public CommonResult<List<UnSelectTopicVo>> remainingSubject(@Valid UnTopicListQuery query) {
        List<UnSelectTopicVo> unSelectTopicVos = topicService.unChooseSubjectList(query.getTeacherNumber(), query.getTeacherName(), query.getYear(), query.getMajorId());
        return CommonResult.success(unSelectTopicVos);
    }

    @Transactional(rollbackFor = Exception.class)
    @GetMapping(value = "/topics/majorSubmit/{subjectId}/{studentId}")
    @LogAnnotation(content = "专业管理员调配题目和学生")
    @RequiresPermissions({"topic:majorSubmit"})
    @ApiOperation(value = "专业管理员调配剩余学生和剩余题目")
    public CommonResult<Object> deploymentSubject(@PathVariable("subjectId")
    ...
    @GetMapping(value = "/topics/majorAdjustList/{year}/{majorId}/{type}/{userName}/{userNumber}")
    ...
    public CommonResult<List<StudentInfoVo>> adjustFirstTeacherAboutSubject(@PathVariable("year") @NotNull Integer year,
    ...
    @GetMapping(value = "/topics/majorSubmitAdjust/{subjectId}/{firstTeacherId}")
    ...
    public CommonResult<Object> modifySubjectMentor(@PathVariable("subjectId") @NotNull Integer subjectId,
                                                    @PathVariable("firstTeacherId") @NotNull Integer firstTeacherId) {
